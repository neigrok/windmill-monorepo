...
// beside this file, and /boot.js, the theme stamp — by middleware in dev and emitted into dist at
// build.
const FONTS = [
...
// Every page in public/ but the undressed ones must link all three.
...
export function staticPageAssets() {
  ...
  for (const page of fs.readdirSync(PAGES).filter((name) => name.endsWith('.html'))) {
    ...
    const missing = [...sheets.keys()].filter((sheet) => !html.includes(`href="/${sheet}"`) && !html.includes(`src="/${sheet}"`));
    if (missing.length) {
