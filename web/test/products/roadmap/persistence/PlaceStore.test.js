...
test('a place round-trips under the layout that saved it', () => {
  ...
  assert.deepEqual(saved, { treeId: 'tree', layout: 'radial', cameraFormat: 'working-zoom-1', camera: { x: 130, y: -230, zoom: 1.1 }, selectedId: 'step', at: saved.at });
...
test('another layout, or a camera saved before the working-zoom format, keeps the tree and selection but no camera', () => {
  ...
  assert.deepEqual(other, { treeId: 'tree', layout: 'radial', cameraFormat: 'working-zoom-1', camera: null, selectedId: 'step', at: other.at });
