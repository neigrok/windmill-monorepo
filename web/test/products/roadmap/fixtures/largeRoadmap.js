// Synthetic roadmaps of a chosen size and shape, for layout benchmarks and stress tests. Deterministic: the same
// (count, shape) always yields the same nodes, labels and order keys.

import { nKeysBetween } from '../../../../src/products/roadmap/sync/fractionalIndex.js';

export const ROADMAP_SHAPES = ['mixed', 'broad', 'deep', 'multiroot'];
...
export function largeRoadmap(count, shape = 'mixed') {
  ...
  if (!ROADMAP_SHAPES.includes(shape)) throw new Error(`Unknown roadmap shape: ${shape}`);
