...
// mintTreeId is crypto.getRandomValues + hex, so these bytes name the remap's fresh id exactly.
...
function browserWith(entries, { hash = '', blobs = {} } = {}) {
  ...
  globalThis.window = {
    ...
    // track()'s debounce would otherwise hold the runner open and post to a restored fetch.
    setTimeout: () => 0,
  };
