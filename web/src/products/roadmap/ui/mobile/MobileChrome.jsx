// Pans reach the canvas between the navigation and camera controls.
...
export function MobileChrome({
  ...
  const recenterRight = desktop && panelOpen ? 408 : tablet && panelOpen ? TABLET_PANEL_WIDTH + 24 : 16;
  ...
  return (
    ...
    >
      ...
      {!listView && (onFocus || showRecenter) && (
        <div
          className="st-zoom-group"
          style={{
            position: 'absolute',
            top: `calc(${SAFE_TOP} + 56px)`,
            flexDirection: tablet || desktop ? 'row' : 'column',
            right: `calc(env(safe-area-inset-right, 0px) + ${recenterRight}px)`,
            pointerEvents: 'auto',
          }}
        >
          {onFocus && <button type="button" className="st-view-action" onClick={onFocus}>Focus</button>}
          <button type="button" className="st-view-action" onClick={onRecenter}>{onFocus ? 'All steps' : 'Recenter'}</button>
        </div>
      )}
