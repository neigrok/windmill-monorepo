...
function EditorStep({ node, state, prerequisites, completedAt, history = [], workspace = EMPTY_WORKSPACE, kinds = [], onOpenLegend = noop, autoFocusName, onRename, onPreviewKind, onRestoreKind, onSetKind, onMarkComplete, onReset, onReveal, onDelete, onPreviewDeleteCost, onClearDeleteCost, onClose, onAddSubtask = noop, onToggleSubtask = noop, onEditSubtask = noop, onDeleteSubtask = noop, onSetNote = noop, onAddLink = noop, onDeleteLink = noop }) {
    ...
    <Card style={{ maxHeight: '70vh', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
      ...
      <div className="st-step-header">
        <div className="st-step-identity">
          {node.icon && (
            <span className="st-step-glyph"><Icon name={node.icon} size={20} /></span>
          )}
      ...
      <div className="st-step-state">
        {state === 'locked' ? (
          ...
          <>
            <span className="st-state-chip" style={{ '--chip-base': hue.base }}>
              <span className={`st-state-dot st-state-dot--${state}`} />
              {state === 'complete' ? 'Complete' : 'Not started'}
            </span>
            <Button
              variant={state === 'complete' ? 'ghost' : 'primary'}
              aria-label={state === 'complete' ? 'Reset step' : 'Complete step'}
              onClick={state === 'complete' ? () => onReset(node.id) : onMarkComplete}
              icon={<Icon name={state === 'complete' ? 'rotate-ccw' : 'check'} />}
            >
              {state === 'complete' ? 'Reset' : 'Complete'}
            </Button>
            {state === 'complete' && completedAt && (
              <div className="st-state-time" title={`Completed ${absoluteTime(completedAt)}`}>
