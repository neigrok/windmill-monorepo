// The box a node reserves for its disc and caption, from the label's length alone — never DOM-measured, so every device
// lays the tree out byte-identically. CSS px of the working view, divided by WORKING_ZOOM into world units.
...
export function footprintOf(label, { root = false } = {}) {
  ...
  return { bodyWu: bodyPx / WORKING_ZOOM, widthWu: widthPx / WORKING_ZOOM, heightWu: heightPx / WORKING_ZOOM, lines };
}
...
export function footprintRect(x, y, footprint) {
  return {
    ...
    minY: y - footprint.bodyWu / 2,
    maxY: y - footprint.bodyWu / 2 + footprint.heightWu,
  };
}
