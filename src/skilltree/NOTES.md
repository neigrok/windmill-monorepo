...
- **Layout is synchronous on the main thread** (~2.3s for the 5k `dagre` pass on the
  "Huge" toggle). It's a one-time cost behind the loading overlay, but a visible freeze.
  Move `LayoutEngine.layout` into a Web Worker when the backend lands — the port already
  isolates it, so it's a drop-in.
...
- **Bundle is ~888 kB** (three + troika). Fine for now; code-split the `skilltree` route
  with a dynamic `import()` if first-paint of the showcase route ever matters.
