...
**Gym Android · the routine opens as a pushed screen with history; canon is a sheet over the list** → open build drift, filed 24 September 2026.
Canon (`813:4935` Routines / Detail, `15-the-routine.md`): a routine row's meta names its first
movements and carries no trained-ago; a tap raises a `ModalBottomSheet` with the name, the plan
without rest lines, `Start workout` and `Edit routine`; no history, no last-trained line, because a
routine never links to any history. Code: `RoutinesScreen.kt:319-338` pushes `GymScreen(title =
"Routine")` with the two actions in a bottom bar, `:362` draws `Last trained …`, `:378` and
`:386-402` draw the History block, `:281-282` draws `Rest m:ss` under each entry, and `:359` counts
movements in the summary line. iOS keeps the pushed screen and its History block (`RoutineScreen.swift:113`,
`:185`); the phones diverge here by the owner's ruling, the web is untouched.

**Gym Android · the routine editor draws Recent changes; canon is name and movements only** → open build drift, filed 24 September 2026.
Canon `820:5572`. Code: `RoutineBuilder.kt:334` reads `store.routineHistory(id)` for the editor and
`:459` draws the `Recent changes` block. iOS draws the same history in its editor
(`RoutineBuilder.swift:134`, `historyOutOfReach`), so it diverges from the Android canon too.

**Gym Android · create movement is name and equipment; canon adds the target block on the same sheet** → open build drift, filed 24 September 2026, owed concern 03 first.
Canon (`814:5104` Create · Routine · Ready, Empty state `669:8424`): name, equipment and a target
block — Sets · Reps · kg stepper rows (`Android / Stepper row`) prefilled 3 × 10 with kg blank, a
readout, Ramp up and Vary by set — under one primary, `Add to routine`, and the movement lands on the
routine with its target. Code: `MovementPicker.kt:386-397` draws Name and Equipment only,
`RoutineBuilder.kt:251` appends the new movement with `draft.adding(it)` and `Program.kt:303` gives it
no scheme, so the row reads `You decide the numbers at the rack.` (`Program.kt:113`) until the target
sheet is opened. The block's own control is concern 03, still open, so this is a design decision
before a build. iOS is the same two fields and `Create and add` (`CreateMovement.swift:111`, `:134`).

**Gym Android · the rest timer is still in Gym settings; canon has none** → open build drift, filed 24 September 2026.
Canon `816:5410`: Units · Notes · Connected log · Account, and nothing about rest anywhere on the
phone. Code: `SettingsScreen.kt:113` (Rest timer row), `:122` (“Rest alerts are paused for this
workout.”), `:136-144` and `:219` (the rest sheet), `:254-301` (Rest alerts with the notification and
exact-alarm setup), `RoutinesScreen.kt:281-282` (Rest line under routine entries). iOS already draws
no rest timer (`Preferences.swift:4`), so on this point the phones converge once Android builds it;
`restSeconds` and `restSound` stay on the wire for the web's dial.

**Gym Android · the Log is a strip and head numbers; canon is the woven timeline** → open build drift, filed 24 September 2026.
Measured against HEAD `79f0ce79`: today's Android commits touched Coach input, the Duplicate removal
and network phases, none the Log, and `apps/android` has no uncommitted change. `LogScreen.kt:83-88`
draws the loaded line and `Trained N of the last 4 weeks` in the head, `:93` the bodyweight reading
row, `:107-110` the `LazyRow` of `ProgressCard`s (`:168-183`), `:138` the Weigh in chip. Canon
(`837:14824` Log / History, `837:14932` Log / Moment expanded; `18-progress.md`, `11-bodyweight.md`):
no head numbers and no strip; a new best for a movement, a weigh-in or a month trained in full lands
as a dated moment card between the sessions, at most one a week; a moment expands in place to the
chart, its window line, best and heaviest and `Open record ›`; a weigh-in moment opens Bodyweight; the
Weigh in chip stays pinned. The record door from a session's movement names already exists in code
(`SessionScreen.kt:428`, `onOpenMovement`) and the readback board now draws it (`Caption · Readback`
`850:14915`). iOS keeps the strip (`LogScreen.swift:61`) and its readback has no movement door.
