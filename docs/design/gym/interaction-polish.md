...
Keep gesture arbitration in one feature-level owner and streaming presentation in one feature-level owner. The store remains responsible for persisted data and domain transitions. Do not broaden these changes into global spacing or text infrastructure without another concrete consumer.
