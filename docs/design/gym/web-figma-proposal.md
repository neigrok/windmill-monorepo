...
- Compact routine cards lead to a draft editor. Targets and conflict recovery use normal pages. Set by set is the main target-editing mode; Every set is an optional shortcut, with a six-set stress case and reachable Save.
- The log puts movement progress above workout history. Record back navigation preserves entry from the log or a workout.
- Saved workout correction has its own set editor. Actual workout entry requires independent weight and reps for every set, with added or removed sets and added or substituted movements. Planned targets are a reference and do not constrain actual values. Past workout shows independent set rows with Add set and Remove controls, a compact date/time row, and inline scrolling on narrow web. Dense actual-set and per-set target tables use plain editable numbers with a subtle focus underline; short forms retain conventional inputs. Kind is omitted.
- Coach displays every proposed change inline in the conversation, with Apply and Dismiss directly beneath it. The same message becomes an applied or dismissed receipt; there is no Review button or review modal. Notes have separate existing-note and blank-note screens.
