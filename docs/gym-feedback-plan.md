...
Status: implementation and review are complete; PR #3 is merged with the tested Git tree. Compatible backend deployment and read-only public endpoint checks passed. Web deployment and the final Android 0.9.0 release remain. Tests and automation use deterministic LLM fakes/fixtures only; actual-model interaction is manual and local only with a user-provided local key. The [execution log](gym-feedback-execution.md) records evidence and remaining delivery gates.
...
Release only after the planned feedback work, supplied prompt, adversarial review, simplification/fix pass, compatible backend rollout and native verification are complete. The selected version is 0.9.0; its workflow-generated `versionCode` must exceed the supported 0.8.2/code76 build. Sign locally with the retained encrypted key, verifying its certificate against `release-signing.json` and the supported installed build. Private signing material stays outside CI. Signing, clean installation, in-place upgrade and publication remain pending.
...
- Local backend verification uses an isolated database and deterministic Anthropic-protocol fixtures. Compatible backend deployment and read-only public endpoint checks passed without model calls. Web Deploy is temporarily disabled and will be restored and dispatched; public streaming is not claimed as verified by those read-only checks.
