...
Status: execution in progress. The [execution log](gym-feedback-execution.md) records implementation, review and verification evidence; unfinished acceptance cases remain requirements.
...
Acceptance: real model output appears before the turn completes in the production-shaped local stack. Tool-assisted replies retain correct receipts. Stop/cancel, disconnect/reconnect, client backgrounding, proxy timeout and provider failure do not duplicate text or tool effects. Scrolling earlier content is stable while new deltas arrive. A failed response preserves partial text and completed actions truthfully, and retry targets the same operation. Deploy the compatible server contract before clients require streaming; confirm proxy buffering and timeout behavior before rollout.
...
Release only after the planned feedback work, supplied prompt, adversarial review, simplification/fix pass, compatible backend rollout and native verification are complete. Choose the next version name and verify the workflow-generated `versionCode` exceeds the supported installed build. Sign locally with the retained encrypted key, verifying its certificate against `release-signing.json` and the supported installed build. Private signing material stays outside CI. An unresolved signing mismatch or unfinished scope keeps this release pending.

Follow the existing [Android release process](../apps/android/README.md#ci-and-releases) and [workflow](../.github/workflows/android.yml): build/test, produce the unpublished signing input through `workflow_dispatch` or an `android-v*` tag, and verify source/run provenance before local signing with `tools/release.py finalize`. Verify a fresh installation, then confirm an in-place update preserves local workouts, routines, settings and queued offline sets; include the revised Coach and workout paths in the release smoke test. Publish the accepted APK, digest, provenance and accurate release notes to the matching GitHub Release, then download and verify the public assets. Record the release URL and verification on `gym-feedback-android-release`. Distribution is by sideload.
...
## Current contracts and remaining inputs
