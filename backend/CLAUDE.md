...
`platform/` is the product-neutral half — auth, oauth, billing, the MCP engine, email, the AI spend
meter (`domain/AiUsage`, `domain/AiFuse`, `ports/AiUsageRepository`: every vendor call is priced and
recorded, and the ceilings read the same rows the owner page does), telemetry, access, the HTTP host.
...
`adapters/` (one subfolder per messy edge it has — `http` and `postgres` everywhere, plus
...
`RUNNING.md` is the local walkthrough. `deploy/README.md` is the production runbook. `SPEC.md` is
the founding design document — read it for the model, not for what exists.
...
- Local `DATABASE_URL` is the unix socket (`postgresql:///windmill?host=/tmp`), which costs no
  ephemeral port. Production keeps TCP; the compose network gives it no choice.
