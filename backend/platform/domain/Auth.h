...
// Wall-clock milliseconds since the Unix epoch, matching Clock::nowMs(). Distinct from Hlc:
// sessions and links expire against real time, not logical time.
...
// The name strangers may see: a name derived from an email address reads as no name at all.
...
struct AuthPolicy {
  ...
  // The typed code lives on the link's row, sharing its lifetime and its single use. A million
  // states is the whole space, so maxCodeAttempts, not the digest at rest, is the defense.
...
// The instant a close becomes final: the grace's end, and the close of the undo window.
...
// Resolved against the newest live code row for the address; everything dead answers noLiveCode.
...
// The stored spelling is the `provider` column's check constraint, so toString/parse are the wire
...
// field that may resolve an account on its own. `name` is present only when the provider sends it
// (Apple, on the first authorization only), so it seeds a new account and never overwrites one.
...
// Apple's Hide My Email: verified and stable for this app, but a relay nobody else can recognise
// the human by. Tested on the published suffix, behind the provider's own claim.
bool isPrivateRelay(const Email& email);

// How far a provider's address can be trusted to name the human:
//   crossDoor — verified and real, so it resolves the account they already have on the web
//   appOnly   — a relay: re-finds the same person here only, so offer the link door instead
