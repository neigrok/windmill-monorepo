...
// Nudge storage + the sweep's seam over Postgres. Stateless but for the connection string. dueNow
// is one indexed range scan over journal_nudge_due; claimDay is an INSERT ... WHERE
// EXISTS(still-eligible) ON CONFLICT DO NOTHING RETURNING that also clears next_due_at, so the
// ledger PK is the daily mutex and a served instant can never fire twice. All instants cross as
// epoch-ms bigints; timestamptz lives only inside this file (to_timestamp / extract epoch), so no
// C++ calendar maths runs — the pqxx readers differ between macOS and CI Linux.
