...
// claimDay is an INSERT ... WHERE EXISTS(still-eligible) ON CONFLICT DO NOTHING RETURNING that also
// clears next_due_at, so the ledger PK is the daily mutex and a served instant cannot fire twice.
// All instants cross as epoch-ms bigints; timestamptz lives only inside this file, because the pqxx
// readers differ between macOS and CI Linux.
