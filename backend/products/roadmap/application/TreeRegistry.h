...
// The per-user tree registry: create, list, rename, and delete the roadmaps a caller owns. A
// repo-direct read model — it never opens a room, so the list is save-consistent (an unsaved
// in-flight edit doesn't move a row). Rename must stay coherent with a live room, so it hands the
// apply to RoomRegistry under the tree's strand.
class TreeRegistry {
public:
  ...
  // untouched. `retired` is the caller's OWN soft-deleted id: the row outlives the delete and still
  // spoke for the id, so the create cannot land, but answering `taken` would send the claim path to
  // re-plant under a fresh id and resurrect a tree its owner deliberately deleted. `taken` is only
  // ever somebody else's: another account's tree, live or retired, and an unowned one.
  ...
  // unowned tree is nobody's, not somebody else's; the surface answering maps them back onto
  // WriteRefusal for its code and sentence.
