...
// Starts and carries a tend run (domain/Tending.h). A run is a JOB, not a stream: `start`
// validates, persists, and hands the agent loop to a private worker thread, so the request returns
// while the loop carries on server-side. The run's state lives in Postgres and the edits land
// through the tree's rooms, so a phone that comes back reads the finished run off
// `TendRunRepository`, never a half-done tree.
//
// The runs execute on a small POOL of private trantor event-loop threads (each blocking agent loop
// runs tens of seconds, so a single thread would serialize concurrent tends), never on a drogon
// request loop. An exception escaping a worker becomes a `failed` run, never a crash.
...
public:
  ...
  // over the account's AI budget, or an unusable prompt) is persisted as a `refused` run and returned
  // without ever starting work. Otherwise a `running` run is persisted and its id returned
  // immediately, with the loop already queued on the worker thread. `email` is the second binding
  // the plan lookup reads.
  TendRun start(const TreeId& tree, const UserId& caller, const std::string& email,
                const std::string& prompt);

  // The meter + receipts ledger read. Always available to a signed-in caller, even while tending is
  // dark — a free account simply reads 0 / 30.
  TendingSummary summaryFor(const UserId& caller, const std::string& email);

  // The catch-up read a returning phone makes. Returns the run only when `caller` owns it — a run
  // that is absent and one that belongs to someone else are the same nullopt, so the id can never
  // be an oracle for another account's runs.
