...
struct PresenceBus {
  ...
  // A progress change is private to one user, so it fans out only to that user's own live sessions
  // on the tree (their other tabs, and a browser watching an agent's edits) — never to
  // collaborators. The registers just recorded, stamps and receipt instants intact, in the same
  // frame shape the graft serves. A no-op where there are no sockets (stdio/HTTP MCP).
...
// their own rooms against the shared Postgres with nobody subscribed. Durability is the database —
// the web server picks an agent's edits up on its next room load.
