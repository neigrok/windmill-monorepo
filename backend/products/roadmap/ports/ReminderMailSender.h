...
// The roadmap-owned reminder mail port ('reminder'). It lives beside the product that words the
// mail, so the platform mail seam never names a reminder. Same asynchronous contract as the
// platform's EmailSender: done fires exactly once with the delivery verdict, and the sweep only
// stamps a row as sent on a true.
