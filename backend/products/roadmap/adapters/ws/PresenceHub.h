...
struct Principal {
  ...
  const UserId user;
  // The display name this connection may show to others — empty for a guest, and empty for an
  // account still wearing the name derived from its address.
...
// Ephemeral cursor/selection, never persisted and never in the op log. Each participant's latest
// state is buffered and flushed to the tree's other participants at ≤ 20 Hz, latest-wins per
// actor, deltas only. Colour is assigned here, one hash-derived hue per actor; an account that
// chose a display name wears it, everyone else gets a generated traveller's name.
class PresenceHub {
public:
...
private:
  ...
  struct Member {
    UserId actor;
    // What this member is called ON THE WIRE: a seat number minted per room, never the real
    // account id, since presence fans out to every co-viewer including anonymous strangers.
    std::string seat;
    std::string name;
    std::string color;
  ...
  // The seat counter lives beside the members so it resets with them.
  struct Room {
