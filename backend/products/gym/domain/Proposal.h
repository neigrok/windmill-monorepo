...
// What class of object a write touches.
...
// Pending until the lifter settles it; every other state is terminal. A pending proposal becomes
// `superseded` when the routine moves underneath it — nothing is merged over the top.
...
// The conversation a proposal was minted in.
struct AskThreadTag;
using ThreadId = Id<AskThreadTag>;

// `connection` and `agent` are empty: `ToolCaller` carries nothing that names one connection apart
// from another. `thread` is absent from the MCP door, and absent once the lifter deletes the thread
// an Ask proposal came from — both mean there is nothing here to open.
...
// `loggedSets` is filled on READ and stored nowhere, for a `removed` row alone.
...
// Proposed lines are matched to base lines by movement, first unmatched first, so a movement
// appearing twice keeps both of its lines.
std::vector<RoutineChange> changesBetween(const std::vector<RoutineEntry>& base,
                                          const std::vector<RoutineEntry>& proposed);

// Every row that moves, one for a renamed routine, and one for a run the proposal reorders. A
// `kept` row is not counted, but a reorder of kept rows is.
...
// caller sent is compared and nothing the store decided — state, mint instant and read-time
