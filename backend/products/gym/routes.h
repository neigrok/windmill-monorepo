...
//
// `coachService` is the ONE nullable field, and null is the shipped default. It is the in-app panel
// in front of those same tools, and it needs a vendor key nobody is obliged to set — so a deployment
// without one gets a null here and the route below is never registered at all. That is the stance
// every vendor-backed feature in this repo takes: dark means absent, never a button that answers 503.
struct GymDeps {
