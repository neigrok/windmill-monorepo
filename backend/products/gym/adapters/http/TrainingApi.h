...
// The training log over REST — everything that reads or writes the log: workouts, sets, the
// per-movement reads off it, the statistics, the CSV of sets, and the coach share. It is one of five
// HTTP adapters that mirror the five aggregate ports (CatalogApi, ProgramApi, PreferencesApi,
// ThreadsApi are the other four; routes.cpp is the one mount), and it is the one that takes
// `appBaseUrl` — a share's link is the only place gym prints the browser app's origin.
//
// Every handler that reads the log resolves the caller first and 401s before touching storage, and
// every one of those reads and writes is scoped to that caller — absent is byte-identical to
// forbidden on all of them.
//
...
//
// THE STATUS LADDER BELOW HOLDS FOR ALL FIVE ADAPTERS, and it is written once, here, because it is
// the whole contract with the client's flush queue, which branches on the status and the machine
// word beside it (ARCHITECTURE §6): 400 is the client's and terminal — an unreadable body, an
// instant outside the wire's bounds, a set or a routine entry naming no known movement; 404 is the
// fact that a session, a routine or a movement named IN THE PATH is absent, which is byte-identical
// to it being another account's — `/v1/gym/last?exercise=` keeps its 400 `unknown-exercise` because
// the movement is an argument to a read about the log there, not the thing the path asks for; 409 is
// the family about something already spent or already running — a session, set, routine or movement
// id that is taken, the start that would have joined a live workout, and the discard of a workout
// still being logged into; 500 is the server's and retryable — a storage failure rides past the
// handlers' `catch (InvalidTraining&)` to the house exception handler on purpose, because a queue
// told "your set is malformed" by a five-second lock wait drops it forever.
//
// Sixteen refusals across the five carry a machine word under `code`, because their repairs differ
// and prose is not a contract: session-id-taken · session-already-open · session-finished ·
// session-open · set-id-taken · set-deleted · routine-id-taken · exercise-id-taken ·
// unknown-exercise · set-not-found · fix-unreadable · preferences-unreadable · unknown-unit ·
// rest-target · proposal-superseded · proposal-settled.
...
//
// The correction's two routes keep a tighter rule than the rest of this file: EVERY refusal they can
// make carries a code. A correction rides the phones' offline queue like every other write, and a
// queue branches — `set-not-found` is terminal (the row is gone, drop the pending edit),
// `fix-unreadable` is terminal and a bug in the client; neither is a sentence anything should ever
// match on.
class TrainingApi {
public:
  ...
  // can draw. It hangs off the catalog's path and lives here because it is a read OF THE LOG.
