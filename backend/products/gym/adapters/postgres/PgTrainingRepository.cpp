...
std::vector<AskThread> PgTrainingRepository::threads(const UserId& user) {
  PgLease conn{*pool_};
  pqxx::work txn{*conn};
  return threadsUnder(txn, user, "t.asked_at DESC, t.id DESC", kThreadList);
}

std::vector<AskThread> PgTrainingRepository::allThreads(const UserId& user) {
...
std::optional<AskThread> PgTrainingRepository::thread(const UserId& user, const ThreadId& id) {
...
ThreadOpenOutcome PgTrainingRepository::openThread(const UserId& user, const ThreadId& id,
...
void PgTrainingRepository::appendTurns(const UserId& user, const ThreadId& id,
  ...
  // instead of racing for the same number — the lock order every write in this module keeps, taking
  // the parent row first.
  PgLease conn{*pool_};
  pqxx::work txn{*conn};
  pqxx::result locked = txn.exec_params(
...
void PgTrainingRepository::discardEmptyThread(const UserId& user, const ThreadId& id) {
...
bool PgTrainingRepository::deleteThread(const UserId& user, const ThreadId& id) {
...
std::vector<ExportedThreadTurn> PgTrainingRepository::exportedThreadTurns(const UserId& user) {
