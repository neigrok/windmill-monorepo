...
namespace {
// A known-good vector computed independently (python hmac-sha256 over "{id}.{ts}.{body}", keyed by the base64-DECODED tail of the secret, digest base64-encoded).
...
// The secret is printed with a whsec_ prefix and signs with the BYTES its base64 tail decodes to, so the prefixed and bare forms must key identically.
...
// Both the id and the timestamp are inside the signed payload, so neither can be swapped for another value.
...
// A destination mid-rotation signs with both secrets and sends a v1 entry for each, space separated; matching either is enough.
...
// An unversioned or v2-labelled entry carrying our digest is not a v1 signature.
