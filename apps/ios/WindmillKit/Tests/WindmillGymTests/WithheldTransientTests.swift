...
// holds one line only from 410 points up. On a 390 it takes a second line, which is why the detail is
...
final class WithheldTransientTests: XCTestCase {
    ...
    func testTheConversationsDetailRunsOnRatherThanBeingCutOffMidWord() async {
        ...
        let above = await height(detail: WithheldWords.threadDetail, width: 410)
        let below = await height(detail: WithheldWords.threadDetail, width: 408)
        ...
        XCTAssertEqual(above, oneLine, "one line from 410 points up")
    ...
    func testTheRoutinesDetailIsOneLineOnEveryPhone() async {
        ...
        for width in [430.0, 410.0, 393.0, 390.0, 375.0, 360.0] {
